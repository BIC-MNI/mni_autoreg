function [offsets,total,w24,w16,w8,w4] = do_warp( disp_flag);
...
%-------------------  load avg model profile  ------------------- 
load avg_profile;

x = avg_profile(:,2);
vsize = avg_profile(2,2) - avg_profile(1,2);
model = avg_profile;
...
% ------------------- set up offsets and deformation field.
...
plot(model(:,2),model(:,1),'g'); hold on
plot(warped(:,2),warped(:,1),'y');
ylabel('intensity');
xlabel('spatial position (mm)');
title('model and subject data');
drawnow;
hold off;
