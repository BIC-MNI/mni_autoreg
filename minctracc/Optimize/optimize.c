...
private float fit_function(float *params) 
{
  ...
  if (Ginverse_mapping_flag)
    build_inverse_transformation_matrix(mat, cent, trans, scale, rots);
  else
    build_transformation_matrix(mat, cent, trans, scale, rots);
				...
				Arg_Data *globals)
{
  ...
  case TRANS_LSQ12: 
    ndim = 12;
    (void) fprintf(stderr, "12 parameter transformation not yet supported\n");
  ...
  if (stat) {
    ...
    p[1][4]=globals->trans_info.rotations[0] * DEG_TO_RAD; 
    p[1][5]=globals->trans_info.rotations[1] * DEG_TO_RAD; 
    p[1][6]=globals->trans_info.rotations[2] * DEG_TO_RAD;
