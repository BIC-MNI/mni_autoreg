...
VIO_General_transform* minctraccScrap ( VIO_Volume source, VIO_Volume target, VIO_Volume sourceMask, VIO_Volume targetMask, VIO_General_transform *initialXFM, int argc, char* argv[] )
{
  VIO_Status status;
  VIO_General_transform tmp_invert;
  VIO_Transform *lt, ident_trans;
  int parse_flag, measure_matlab_flag, sizes[3],i,num_features;
  VIO_Real min_value, max_value, step[3];
  char *comments = history_string( argc, argv );
  FILE *ofd;
  VIO_Real obj_func_val;
  float quat4;
  ...
  /* Call ParseArgv to interpret all command line args (returns TRUE if error) */
	...
	measure_matlab_flag = 0;
  parse_flag = ParseArgv(&argc, argv, argTable, 0);

  /* Something went wrong with parsing the arguments */
...
// ROBB - kill this for final
  main_args->filenames.output_trans = argv[3];


// ROBB if initialXFM put initialXFM into main_args->trans_info.transformation
// and set main_args->trans_info.use_default = FALSE;

  if (!initialXFM) {
	fprintf(stderr,"Setting initial XFM\n");
	main_args->trans_info.transformation = initialXFM;
	main_args->trans_info.use_default = FALSE;
	
/*	main_args->trans_flags.estimate_center = FALSE;
	main_args->trans_flags.estimate_scale = FALSE;
	main_args->trans_flags.estimate_rots = FALSE; 
	main_args->trans_flags.estimate_quats = FALSE;
	main_args->trans_flags.estimate_trans = FALSE; */
  ...
  if (main_args->trans_info.transformation == (VIO_General_transform *)NULL) { 
	...
	fprintf(stderr,"Setting initial XFM to Identity\n");
  ...
  // ROBB - probably have to switch this around so that the copy is used (modified) by the optimization
...
/*  (void)fprintf(stderr,"WRITING TO TRANSFORM FILE %s\n", main_args->filenames.output_trans);
...
// 		ROBB - will probably need to clean up the freeing 
...
//  FREE(main_args->trans_info.orig_transformation);
  ...
  return( main_args->trans_info.transformation );
}
